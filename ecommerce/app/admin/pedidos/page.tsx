...
async function getPedidos(status?: string, pagina = 1) {
  const params = new URLSearchParams({ pagina: String(pagina) })
  if (status) params.set('status', status)

  const res = await fetch(
    `${process.env.NEXTAUTH_URL ?? 'http://localhost:3000'}/api/admin/pedidos?${params}`,
    { cache: 'no-store' }
  )
...
export default async function AdminPedidosPage({ searchParams }: Props) {
  ...
  const data         = await getPedidos(statusFiltro, paginaAtual)
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        ...
        {data.totalPaginas > 1 && (
          <div className="px-4 py-3 border-t border-gray-100 flex items-center justify-between text-sm text-gray-500">
            <span>
              {data.total} pedidos · página {data.pagina} de {data.totalPaginas}
            </span>
